...
const stats = computed(() =>
  currentUser.value ? getStats(currentUser.value.id) : null,
);
  ...
  <AtmScreenLayout :warn="false">
    <div class="flex-1 flex flex-col items-center justify-center gap-5 px-8 py-4 overflow-y-auto">
      ...
      <template v-if="!currentUser">
        <p class="text-white/60 text-base text-center">Aucun utilisateur sélectionné</p>
        <button
          class="px-6 py-4 rounded-xl text-white font-bold tracking-wide transition-colors border"
      ...
      <template v-else>
        <!-- Nom -->
        <h1 class="text-2xl font-black tracking-wide text-center" style="color: #f0c040">
          {{ currentUser.name }}
        </h1>
        ...
        <!-- Streak -->
        <div class="w-full max-w-xs rounded-xl px-5 py-4 text-center" style="background: rgba(0,0,0,0.3)">
          <p class="text-xl font-bold" style="color: #f0c040">
            {{ stats.currentStreak }} succès consécutif{{ stats.currentStreak !== 1 ? 's' : '' }}
          </p>
          <p v-if="stats.currentStreak >= 5" class="font-bold text-sm mt-1" style="color: #00d4a0">
            Félicitations — série exceptionnelle !
          </p>
        </div>
